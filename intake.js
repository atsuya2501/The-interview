...
let LAST_CONFIDENCE = '';     // 回答率（信頼度）サマリ
...
// 設問が回答済みか（期間=数値入力あり / 複数選択=1つ以上 / それ以外=選択あり）
function qAnswered(q) {
  if (q.input_type === 'duration') { const el = document.getElementById(q.id); return !!(el && el.value !== ''); }
  if (q.input_type === 'multi_select') return checkVals(q.id).length > 0;
  return radioVal(q.id) !== null;
}
...
function deriveAndWrite() {
  ...
  LAST_LEVEL_BASIS = LAST_ACUTE_PHASE = LAST_REDFLAG_TIER = LAST_TISSUE_WARNING = LAST_CONFIDENCE = '';
  ...
  // ④ 回答率（信頼度）: 未回答が多いと導出の信頼度が下がることを明示
  const allQs = FLOW.flow.filter(s => !s.redirect).flatMap(s => s.questions || []);
  const answered = allQs.filter(qAnswered).length;
  const total = allQs.length;
  const pct = total ? Math.round((answered / total) * 100) : 0;
  const label = pct >= 80 ? '高' : pct >= 50 ? '中' : '低';
  const s5ans = lvDefs.filter(qAnswered).length;
  LAST_CONFIDENCE = `${answered}/${total}項目（${pct}%）・信頼度${label}`
    + (levelAnswered && s5ans < 3 ? `／痛みレベルは回答${s5ans}/9で参考程度` : '');
    ...
    function finish() {
      ...
      sm.innerHTML = keys.length
        ? `<p class="lead">カルテに反映しました。</p><ul class="hit-list">${keys.map(k =>
           ...
           ${LAST_CONFIDENCE ? `<p class="hint">回答の網羅度：${LAST_CONFIDENCE}</p>` : ''}
           ${/信頼度低/.test(LAST_CONFIDENCE) ? `<div class="alert yellow"><strong>⚠ 回答が不足しています</strong><p>未回答項目が多く、判定の信頼度が低い状態です。可能なら追加で回答してください。</p></div>` : ''}
